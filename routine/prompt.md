...
3. Read sites.json AND the user's profile ($AUTORECLAIM_DATA_DIR/profile.json — it has
   ...
   breach names to companies; ignore generic entries like "scraped"). Write ONLY the ones
   ...
   claim_url — never affiliate/"bonus cash" links some aggregators carry. Be generous on
   recall (the user confirms before filing); skip clear non-matches and anything past its
   deadline.
