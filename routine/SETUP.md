...
- `autoreclaim-public` — code (this repo)
...
1. Push BOTH repos to GitHub:
   - `autoreclaim-public` (code)
   - `autoreclaim-data` (your profile + queue — keep PRIVATE)
2. Go to https://claude.ai/code/routines → New routine → Remote.
3. Prompt: paste `routine/prompt.md`.
4. Repositories: add BOTH `autoreclaim-public` and `autoreclaim-data`. For
   `autoreclaim-data`, enable "Allow unrestricted branch pushes" so the routine can
   commit `queue.jsonl` back. Leave `autoreclaim-public` read-only (default).
5. Environment:
   - Network access → **Custom**. Allowed domains:
     ...
     api.xposedornot.com (the free data-breach lookup — without it the breach scan,
     the highest-confidence matcher, silently fails in the cloud)
